/**
 * A base class for classes that must be constructed and initialized asynchronously.
 */
export declare abstract class AsyncCreatable<O> {
    /**
     * Asynchronously constructs and initializes a new instance of a concrete subclass.
     *
     * @param {P} [options] An options object providing initialization params to the async constructor.
     */
    static create<P, T>(this: {
        new (): T & AsyncCreatable<P>;
    }, options?: P): Promise<T>;
    protected constructor();
    /**
     * Asynchronously initializes newly constructed instances of a concrete subclass.
     *
     * @param {P} [options] An options object providing initialization params.
     */
    protected abstract init(options?: O): Promise<void>;
}
